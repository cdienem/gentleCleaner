...
# Checks if a directory is a Relion project by looking for a default_pipeline.star
# Beware, this can also be true for job folders! -> see readRelionProjectJobs
...
# harsh: True = perform harsh cleaning
# dry (True/False): either return just a file list or actually do clean (e.g. move things to trash)
# absolute: only when doing dry runs, True means print absolute paths, False means print paths relative to project directory
# Returns a list of files to be cleaned (or that were moved to Trash when harsh = True)

	# Get job type
	job_type = job.split("/")[0]
	
	# Do not clean these job types
	...
	if job_type == "MotionCorr":
		...
		# Use a list as it stays ordered, so all iterations except last can be cleaned
		iteration_files = []
	...
	if dry:
		...
		# Make delete_files with relative paths for moving them to Trash
		...
		# Give choice what should be returned
		if absolute:
			return delete_files
		else:
